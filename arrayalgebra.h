...
constexpr auto operator<<(std::ostream &os,
                          std::array<T, N> const &a) -> std::ostream & {
...
// v1 + v2
template <typename T, std::size_t N>
constexpr auto operator+(std::array<T, N> const &a1,
                         std::array<T, N> const &a2) -> std::array<T, N> {
...
constexpr auto operator+(std::array<std::array<T, M>, N> const &m1,
...
// v * c
template <typename T, std::size_t N>
constexpr auto operator*(std::array<T, N> const &a,
                         T const v) -> std::array<T, N> {
  std::array<T, N> result{};
  for (size_t j = 0; j < N; j++) {
    result[j] = v * a[j];
  }
...
template <typename T, std::size_t N, std::size_t M>
constexpr auto operator*(std::array<std::array<T, M>, N> const &m,
                         T const &v) -> std::array<std::array<T, M>, N> {
  std::array<std::array<T, M>, N> result{};
  for (size_t i = 0; i < N; i++) {
    for (size_t j = 0; j < M; j++) {
      result[i][j] = v * m[i][j];
...
constexpr auto operator*(T const v,
                         std::array<T, N> const &a) -> std::array<T, N> {
  std::array<T, N> result{};
  for (size_t j = 0; j < N; j++) {
    result[j] = v * a[j];
  }
...
// m * v
template <typename T, std::size_t N>
constexpr auto operator*(std::array<std::array<T, N>, N> const &mat,
                         std::array<T, N> const &vec) -> std::array<T, N> {
  std::array<T, N> result{};
  for (size_t i = 0; i < N; i++) {
...
// m1 * m2
template <typename T, std::size_t N, std::size_t M>
constexpr auto operator*(std::array<std::array<T, N>, M> const &m1,
                         std::array<std::array<T, M>, N> const &m2)
...
// v1 dot v2
template <typename T, std::size_t N>
constexpr auto dot(std::array<T, N> const &a1,
                   std::array<T, N> const &a2) -> T {
  return std::inner_product(a1.begin(), a1.end(), a2.begin(),
                            static_cast<T>(0));
}
...
// v1 cross v2
template <typename T>
constexpr auto cross(std::array<T, 3> const &v1,
                     std::array<T, 3> const &v2) -> std::array<T, 3> {
